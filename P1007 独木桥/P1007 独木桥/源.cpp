...
//
//然后我们就可以把士兵分开了。比方说有一个士兵在位置3，开始时向右，
//那么一定有一个士兵在两秒后在位置5。虽然这两个家伙可能不是同一个人，但由于士兵都是相同的，我们可以认为他们相同。

int main() {
	int l, n;
	scanf("%d %d", &l, &n); int maxn =0, minn =0,a=0;   //不要随意赋值，n是可以为0的，这样的话，打印的就是原来的初始化的maxn,minn，所以必须初始化为0
	for (int i = 1; i <= n; ++i) {
		scanf("%d", &a);
		maxn = max(maxn, max(l + 1 - a, a));
